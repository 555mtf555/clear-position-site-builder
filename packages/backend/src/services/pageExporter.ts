...
export async function exportSite(site: Site): Promise<ExportResult> {
  ...
  const manifestPath = path.join(exportDir, "export-manifest.json");
  const manifest = createManifest(context, relativeExportPath(exportDir, manifestPath), { siteId: site.id });
  await writeManifest(manifestPath, manifest, context);
...
<html lang="en">
  <head>
    ...
    <style>${exportCss(context.brandKit)}</style>
  </head>
  <body>
...
function exportCss(brandKit: BrandKit | null): string {
  ...
  return `
:root{--cpsb-primary:${primary};--cpsb-secondary:${secondary};--cpsb-accent:${accent};--cpsb-background:${background};--cpsb-text:${text};--cpsb-muted-text:${muted};--cpsb-button-background:${buttonBackground};--cpsb-button-text:${buttonText};--cpsb-border-radius:${radius};--cpsb-font-family:${fontFamily}}
*{box-sizing:border-box}body{margin:0;font-family:var(--cpsb-font-family);color:var(--cpsb-text);background:var(--cpsb-background)}a{color:inherit}
